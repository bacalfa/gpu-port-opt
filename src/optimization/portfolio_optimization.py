...
class GPUPortfolioAccelerator:
    ...
    def compute_covariance(returns, use_gpu: bool = True):
        ...
        if use_gpu and GPU_AVAILABLE:
            try:
                returns_gpu = cudf.from_pandas(returns) if isinstance(returns, pd.DataFrame) else returns
                # cuDF covariance
...
class PortfolioOptimizer:
    ...
    def mean_variance_optimization(self,
        ...
        model.w = pyo.Var(model.ASSETS, domain=pyo.NonNegativeReals, bounds=(min_weight, max_weight))
        ...
        def objective_rule(m):
            ...
            portfolio_variance = sum(
                sum(model.w[i] * model.w[j] * covariance_matrix.iloc[i, j]
        ...
        if target_return is not None:
            def target_return_rule(m):
                return sum(model.w[i] * expected_returns.iloc[i] for i in model.ASSETS) >= target_return
        ...
        logger.info(f"✓ Optimization completed - Return: {portfolio_return:.4f}, Vol: {portfolio_volatility:.4f}")
    ...
    def maximum_sharpe_ratio(self,
        ...
        def objective_rule(m):
            return sum(
                sum(model.y[i] * model.y[j] * covariance_matrix.iloc[i, j]
                    for j in model.ASSETS)
                for i in model.ASSETS
            )
    ...
    def risk_parity_portfolio(self,
                             covariance_matrix: pd.DataFrame,
                             target_risk_contributions: Optional[pd.Series] = None) -> Dict:
        """
...
class RegimeAwareOptimizer:
    ...
    def optimize_multi_regime(self,
        ...
        expected_returns = sum(
            ...
            for regime_id in returns_by_regime.keys()
        )
        ...
        for regime_id in returns_by_regime.keys():
            expected_covariance += regime_probabilities[regime_id] * regime_stats[regime_id]['covariance']
    ...
    def optimize_regime_adaptive(self,
        ...
        for t in range(lookback_window, len(returns)):
            ...
            try:
                result = self.optimizer.mean_variance_optimization(
                    ...
                    covariance_matrix=self.gpu.compute_covariance(regime_returns, use_gpu=self.optimizer.use_gpu),
                    risk_aversion=risk_aversion
                )
...
class PortfolioBacktester:
    ...
    def backtest_strategy(self,
                         ...
                         returns: pd.DataFrame,
                         transaction_cost: float = 0.001,
                         rebalance_frequency: int = 21) -> pd.DataFrame:
        """
        ...
        results_df['cumulative_return'] = results_df['portfolio_value'] / results_df['portfolio_value'].iloc[0] - 1
...
def run_backtest(returns: pd.DataFrame, portfolios: Dict[str, pd.Series], show_plots: bool = False):
    ...
    print("\nPerformance Summary:")
    print(metrics_df[['annualized_return', 'annualized_volatility', 
                      'sharpe_ratio', 'max_drawdown']].to_string())
    ...
    ax = axes[0, 0]
    for name, data in backtest_results.items():
        (1 + data['results']['portfolio_return']).cumprod().plot(
            ax=ax, label=name, linewidth=1.5
        )
    ...
    print(f"✓ Saved metrics to results/portfolio_performance_metrics.csv")
...
def main():
    ...
    print("\n5. Regime-Aware Optimization")
    print("-"*70)
    # For demonstration, create dummy regimes
    regimes = pd.DataFrame(index=returns_recent.index)
    regimes['regime'] = (returns_recent.mean(axis=1) > 0).astype(int)  # Simple bull/bear regime
    regime_optimizer = RegimeAwareOptimizer(solver='ipopt', use_gpu=True)
    regime_probs = pd.Series({0: 0.4, 1: 0.6})  # Dummy probabilities
    returns_by_regime = {
        0: returns_recent[regimes['regime'] == 0],
        1: returns_recent[regimes['regime'] == 1]
    }
    regime_result = regime_optimizer.optimize_multi_regime(
    ...
    print(f"Expected Return: {regime_result['expected_return']:.4f}")
    print(f"Volatility: {regime_result['volatility']:.4f}")
    print(f"Sharpe Ratio: {regime_result['sharpe_ratio']:.4f}")
    print(f"Top 5 positions:")
    print(regime_result['weights'].nlargest(5))
    portfolios["Regime-Aware"] = regime_result["weights"]
